...
interface TvDataProductsModalProps {
  code: string
  onClose: () => void
}

export default function TvDataProductsModal({ code, onClose }: TvDataProductsModalProps) {
  useEffect(() => {
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`Productos TvData para código ${code}`}
      onClick={onClose}
    >
      <div
      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-3">
          <div className="flex items-center gap-2">
            <h2 className="text-sm font-bold text-[#2d3436]">Productos TvData</h2>
        ...
        <div className="flex-1 overflow-auto bg-white p-4">
          ...
          {query.isError && (
            <ErrorState message="No se pudo cargar productos TvData. Reintentá luego." />
          )}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
      ...
      <p className="text-sm font-semibold text-slate-700">
        Sin productos TvData registrados
      </p>
      <p className="max-w-sm text-xs text-slate-500">
        No hay productos en el catálogo TvData con código <strong className="font-mono">{code}</strong>. Agregalos en el módulo TvData para que aparezcan acá.
      </p>
    </div>
  )
}

function ProductsList({ items }: { items: TvData[] }) {
  return (
    <ul className="divide-y divide-slate-100">
      {items.map((p) => (
        ...
        >
          ...
          <span className="font-mono text-sm font-bold text-[#2d3436] shrink-0">{p.code}</span>
