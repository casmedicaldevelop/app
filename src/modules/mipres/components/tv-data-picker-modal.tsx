...
interface TvDataPickerModalProps {
...
export default function TvDataPickerModal({ initialSearch = '', onSelect, onClose }: TvDataPickerModalProps) {
  ...
  const query = useQuery({
    queryKey: ['tv-data-picker', debounced],
    queryFn: () => tvDataService.list({ search: debounced, limit: 100 }),
    enabled,
  })

  const items: TvData[] = query.data?.data ?? []
      ...
      >
        <header className="flex items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-5 py-3">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold text-[#2d3436]">Buscar producto en TvData</h2>
        ...
        <div className="flex-1 overflow-auto bg-white">
          ...
          {enabled && query.isError && (
            <ErrorState message="No se pudo buscar en TvData. Reintentá." />
          )}
            ...
            <ul className="divide-y divide-slate-100">
              {items.map((p) => (
                <li key={p.id}>
                  ...
                  >
                    <span className="shrink-0 whitespace-nowrap font-mono text-sm font-bold text-[#2d3436]">
                      {p.code}
                    </span>
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2 py-10 text-center">
      ...
      <p className="max-w-sm text-xs text-slate-500">
        No se encontraron productos TvData para <strong className="font-mono">{query}</strong>. Probá
