...
/** Tabla de registros de entrega del filing. Se renderiza DENTRO de la sección Entrega. */
export function DeliveryHistory({ filing }: { filing: FilingDetail }) {
  const { data, isLoading } = useDeliveries(filing.id)
          ...
          {data.map((d) => (
            <Row
              ...
              onTicket={async () => {
                ...
                try {
                  const { shelfCode } = await filingMipresService.assignShelfCode(filing.id)
                  renderTicket(w, { company, filing, delivery: d, shelfCode })
                } catch {
                  w.document.body.innerHTML =
                    '<p style="font:14px sans-serif;padding:16px;color:#b91c1c">No se pudo generar el ticket.</p>'
                }
              }}
              onFormato={() => openFormatoWindow({ company, filing, delivery: d })}
            />
          ))}
        </tbody>
      </table>
