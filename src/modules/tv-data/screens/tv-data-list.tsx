...
import {
  ...
  Upload, X, ChevronLeft, ChevronRight, Download, Database, ArrowLeft, SlidersHorizontal,
} from 'lucide-react'
import { useTvDataList } from '../hooks/useTvDataList'
import { useDeleteTvData } from '../hooks/useDeleteTvData'
import { useBulkUploadTvData } from '../hooks/useBulkUploadTvData'
import TvDataFormDrawer from '../components/tv-data-form-drawer'
import TvDataFiltersDrawer from '../components/tv-data-filters-drawer'
import type { TvData } from '../types/tv-data.types'
...
async function downloadTemplate(table: string) {
  ...
  const res = await fetch(`${apiConfig.baseUrl}/tv-data/bulk/template`, {
  ...
  a.href = url
  a.download = `plantilla_${table}.xlsx`
  a.click()
  URL.revokeObjectURL(url)
}

function BulkUploadModal({ table, onClose }: { table: string; onClose: () => void }) {
  const fileRef = useRef<HTMLInputElement>(null)
  const [file, setFile] = useState<File | null>(null)
  const bulkUpload = useBulkUploadTvData()
    ...
    >
      <div className="w-full max-w-sm rounded-xl border border-border bg-background shadow-xl">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div>
            <h2 className="text-sm font-semibold text-foreground">Carga masiva — <span className="font-mono">{table}</span></h2>
...
export default function TvDataListPage() {
  ...
  const [drawer, setDrawer] = useState<{ mode: 'create' | 'edit'; record?: TvData } | null>(null)
  const [filtersOpen, setFiltersOpen] = useState(false)

  const { data, isLoading, isError, refetch } = useTvDataList({
    page,
    limit: PAGE_SIZE,
    search: search.trim() || undefined,
  })

  const deleteMutation = useDeleteTvData()
  ...
  return (
    <>
      <div className="rounded-xl border border-border bg-background shadow-sm overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div className="flex items-center gap-3 min-w-0">
            <button
              type="button"
              onClick={() => navigate('/dashboard/tv-data')}
              aria-label="Volver"
              title="Volver"
            ...
            <div className="min-w-0">
              <h1 className="font-mono text-base font-semibold text-foreground truncate">{table ?? 'TvData'}</h1>
              {!isLoading && !isError && (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/30">
                    ...
                    <th scope="col" className="px-4 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-muted-foreground w-40">Cód. inventario</th>
                    <th scope="col" className="px-4 py-2.5 text-right text-xs font-semibold uppercase tracking-wider text-muted-foreground w-32">Precio</th>
                  ...
                  {records.map((record) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs font-mono text-foreground">
                        {record.inventoryCode ?? <span className="text-muted-foreground">—</span>}
                      </td>
                      <td className="px-4 py-3 text-right text-xs font-mono tabular-nums text-foreground">
                        {fmtCop(record.price)}
                      </td>
      ...
      {showBulk && <BulkUploadModal table={table ?? 'tvdata'} onClose={() => setShowBulk(false)} />}
      ...
      {drawer && (
        <TvDataFormDrawer mode={drawer.mode} record={drawer.record} onClose={() => setDrawer(null)} />
      )}
