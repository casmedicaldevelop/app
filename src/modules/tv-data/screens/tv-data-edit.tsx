...
import { useTvDataItem } from '../hooks/useTvDataItem'
...
export default function TvDataEditPage() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const numericId = id ? parseInt(id, 10) : 0
  const mutation = useUpdateTvData(numericId)

  const { data: record, isLoading, isError } = useTvDataItem(numericId)
  ...
  const onSubmit = handleSubmit((data) => {
    const trimmedInventoryCode = data.inventoryCode.trim()
    const payload: UpdateTvDataPayload = {
      code: data.code.trim(),
      name: data.name.trim(),
      inventoryCode: trimmedInventoryCode === '' ? null : trimmedInventoryCode,
      ...
      <form onSubmit={onSubmit} noValidate>
        <div className="px-6 py-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label htmlFor="tve-inventory-code" className="block text-sm font-medium text-foreground">
                Código de inventario
              </label>
              <input
                id="tve-inventory-code"
                autoComplete="off"
            ...
            <div className="space-y-1.5">
              <label htmlFor="tve-price" className="block text-sm font-medium text-foreground">
