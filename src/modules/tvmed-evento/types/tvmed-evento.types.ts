...
export interface TvMedEventoPage {
  data: TvMedEvento[]
  total: number
  page: number
  limit: number
...
export interface CreateTvMedEventoPayload {
  cum: string
  name: string
  value: number
  concentration: string
...
export type UpdateTvMedEventoPayload = Partial<CreateTvMedEventoPayload>

export interface ListTvMedEventoParams {
  page?: number
  limit?: number
  cum?: string
  name?: string
...
export function eventoFiltersToParams(f: EventoFilters): ListTvMedEventoParams {
  const p: ListTvMedEventoParams = {}
...
export interface BulkUploadTvMedEventoResult {
  inserted: number
  total: number
}
