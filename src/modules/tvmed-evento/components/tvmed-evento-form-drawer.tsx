...
import { useCreateTvMedEvento, useUpdateTvMedEvento } from '../hooks/useTvMedEvento'
...
export default function TvMedEventoFormDrawer({ mode, record, onClose }: Props) {
  const createMut = useCreateTvMedEvento()
  const updateMut = useUpdateTvMedEvento(record?.id ?? 0)
