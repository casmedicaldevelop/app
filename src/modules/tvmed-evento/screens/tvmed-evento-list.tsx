...
import TvEventoRowModal from '../components/tv-evento-row-modal'
import {
  useTvMedEventoList,
  useBulkUploadTvMedEvento,
  useUpdatePreviewTvMedEvento,
  useUpdateApplyTvMedEvento,
} from '../hooks/useTvMedEvento'
import { useActiveEventContract } from '@/modules/evento/hooks/useEventContract'
import TvMedEventoFiltersDrawer from '../components/tvmed-evento-filters-drawer'
...
async function downloadUpdateTemplate() {
  ...
  const res = await fetch(`${apiConfig.baseUrl}/tvmed-evento/update/template`, {
...
async function downloadMissing(rows: UpdateMissing[]) {
  const { accessToken } = useAuthStore.getState()
  const res = await fetch(`${apiConfig.baseUrl}/tvmed-evento/update/missing-template`, {
    method: 'POST',
...
function BulkUploadModal({ onClose, onUploaded }: { onClose: () => void; onUploaded: () => void }) {
  ...
  const bulkUpload = useBulkUploadTvMedEvento()
...
function UpdateModal({ onClose, onMissing }: { onClose: () => void; onMissing: () => void }) {
  ...
  const preview = useUpdatePreviewTvMedEvento()
  const apply = useUpdateApplyTvMedEvento()
...
export default function TvMedEventoListPage() {
  ...
  const [viewRow, setViewRow] = useState<TvMedEvento | null>(null)

  const activeFilters = countActiveEventoFilters(filters)
  const { data, isLoading, isError, refetch } = useTvMedEventoList({
    page,
    limit: PAGE_SIZE,
